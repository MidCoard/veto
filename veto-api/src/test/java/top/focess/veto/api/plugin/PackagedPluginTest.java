...
class PackagedPluginTest {
    @Test
    void nativeToolAuthoringLoadsWithOnlyApiVisible() throws Exception {
        try (var isolated = loader()) {
            var type = isolated.loadClass("top.focess.veto.fixture.ApiOnlyTool");
            var tool =
                    assertInstanceOf(
                            ToolDocs.nonNullClass(NativeTool.class),
                            type.getConstructor().newInstance());
            assertEquals(ToolCapability.NETWORK_EGRESS, tool.getCapability());
            assertThrows(
                    ClassNotFoundException.class,
                    () -> isolated.loadClass("top.focess.veto.agent.AgentRunner"));
        }
    }
