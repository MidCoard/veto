...
public final class TaskControlCapabilityImpl implements TaskControlCapability {
    ...
    @Override
    public @NonNull Optional<TaskInfo> status(@NonNull String taskId) {
        var context = CapabilityAccess.require(ToolCapability.TASK_CONTROL);
        return taskManager.status(context.agentId(), taskId);
    }

    @Override
    public @NonNull Optional<TaskInfo> awaitExit(@NonNull String taskId)
    ...
    @Override
    public @NonNull Optional<TaskInfo> stop(@NonNull String taskId) {
