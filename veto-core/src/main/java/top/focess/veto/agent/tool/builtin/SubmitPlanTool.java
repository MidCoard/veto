...
@ToolDoc(
        description =
                "Submit an executable plan: ordered tool steps, generated text, branches and a final STOP.",
        behavior =
        ...
        whenToUse =
                "Use when the next steps and data dependencies are known, especially a sequence with conditional branches or bounded repetition.",
        whenNotToUse =
                "Use ordinary tools while exploring an uncertain task. Reply in text when the task is complete. Call this tool alone; it transfers control to the submitted plan.",
        resultContract =
        ...
        errorsAndEdgeCases =
                "Each action needs a unique id, a label and a type. Finish with STOP. Tool actions require tool, inputs and outputs. Generate actions require prompt and outputs; omitted inputs means no local bindings. outputs is a variable-binding map, never configuration: each key names a new variable and each value selects a result field. Generate exposes message; {\"answer\":\"message\"} binds its text to $answer. Read bound values with $name; action indices are zero-based. Use only catalog tools and respect their argument schemas. Cycles need an exit, and all steps consume budgets. The final result comes from STOP.result_binding; without it the runtime returns accumulated bindings.",
        security =
