...
/**
 * Stable machine-readable code carried by a failed, refused, or interrupted {@link ToolResult}.
 * Each code is a constant of one nested group enum — {@code ToolErrorCode.WORKSPACE.PATH_NOT_FOUND}
 * — so the error aspect is compile-time structure. The wire and persisted representation is the
 * constant {@link #name()} as a plain string; {@link #parse(String)} is the tolerant reader for
 * persisted payloads, returning null for unknown names.
 */
    ...
    final class ByName {
        ...
        private static @NonNull Map<@NonNull String, @NonNull ToolErrorCode> index() {
            ...
            ToolErrorCode[][] groups = {
                Nullness.requireNonNull(VALIDATION.values()),
                Nullness.requireNonNull(WORKSPACE.values()),
                Nullness.requireNonNull(TASK.values()),
                Nullness.requireNonNull(NETWORK.values()),
                Nullness.requireNonNull(READER.values()),
                Nullness.requireNonNull(POLICY.values()),
                Nullness.requireNonNull(LIFECYCLE.values()),
                Nullness.requireNonNull(RESULT.values()),
                Nullness.requireNonNull(GENERIC.values()),
                Nullness.requireNonNull(GROUP.values()),
                Nullness.requireNonNull(MEMORY.values()),
                Nullness.requireNonNull(MONITOR.values()),
                Nullness.requireNonNull(SESSION.values())
            };
