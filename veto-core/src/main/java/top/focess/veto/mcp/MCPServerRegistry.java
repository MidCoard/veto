...
/**
 * C4 MCP Server Registry - dynamically discovers and mounts local/enterprise MCP Servers. Watches
 * the configured registry path for new server definitions.
 */
@Component
public class MCPServerRegistry {
    ...
    private final ConcurrentHashMap<String, MCPServerDefinition> servers =
            new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, List<MCProtocol>> toolRegistry =
            new ConcurrentHashMap<>();
    ...
    public MCPServerDefinition registerServer(Path definitionFile) {
        try {
            ...
            MCPServerDefinition definition =
                    objectMapper.readValue(content, MCPServerDefinition.class);

            servers.put(definition.getId(), definition);
            log.info(
                    "C4 MCP: Registered server '{}' (type={}, tools={})",
                    definition.getId(),
                    definition.getServerType(),
            ...
            for (var toolEntry : definition.getTools().entrySet()) {
                MCProtocol protocol =
                        new MCProtocol(
                                ...
                                (Map<String, Object>)
                                        toolEntry.getValue().getOrDefault("inputSchema", Map.of()),
    ...
    public static class MCPServerDefinition {
        ...
        public void setWasmModulePath(String wasmModulePath) {
            this.wasmModulePath = wasmModulePath;
        }

        public Map<String, Map<String, Object>> getTools() {
            return tools;
        }

        public void setTools(Map<String, Map<String, Object>> tools) {
            this.tools = tools;
        }
