 ...
 *
 * <p>The focess-command framework only pre-registers {@link DataBuffer DataBuffers} for its
 * built-in converters (String, Integer, Long, Double, Boolean). Any custom converter, such as
 * {@link DataConverter#ofEnum} for {@link ModelTier}, must be paired with a {@link
 * DataCollection.BufferGetter BufferGetter} so the framework can allocate storage for its target
 * class when it builds the {@link DataCollection}. This holder performs that registration once at
 * class-load time and exposes the converter instances for use in {@link CommandArgument
 * CommandArguments}.
 *
