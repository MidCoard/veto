...
class PlanExecutionTest {
    ...
    @Test
    void sameSequentialFileTaskUsesTwoPlanCallsOrThreeOrdinaryCalls(@TempDir @NonNull Path root)
            throws Exception {
        root = root.toRealPath();
        ...
        List<ToolCallEvent> ordinaryTools = new ArrayList<>();
        var planResult =
                plan.submit(
                        "read-plan",
                        task,
        ...
        assertEquals(planResult.message(), ordinaryResult.message());
        assertEquals(
                2,
                planTools.stream().filter(t -> !t.toolName().equals("submit_plan")).count(),
                "plan must execute both real sequential file reads");
        assertEquals(2, ordinaryTools.size());
        assertEquals(2, planRequests.size(), "direct program plus one generation request");
        assertEquals(
                3,
                ordinaryRequests.size(),
                "two sequential observations plus final model response");
        assertTrue(
                planRequests.get(1).tools().stream()
                        .allMatch(t -> t.name().equals("answer_with_citations")));
        assertNotEquals(planRequests.get(0).systemPrompt(), planRequests.get(1).systemPrompt());
        assertEquals(
        ...
        assertTrue(
                ordinaryRequests.get(0).tools().stream()
                        .anyMatch(t -> t.name().equals("submit_plan")));
        String planPrompt = planRequests.get(0).systemPrompt();
        String ordinaryPrompt = ordinaryRequests.get(0).systemPrompt();
        System.out.println(
                "Plan comparison: plan prompt chars="
                        + planPrompt.length()
