...
class DefaultUniformLLMCallerTest {
    private VetoRequest request(ProviderType type) {
        return new VetoRequest("sys", "user", List.of(), type, "model-x", "openai-key", null);
    }

    private LlmEgress egressReturning(String apiKey) {
    ...
    void delegatesToSupportingProvider() {
        ...
        VetoResponse expected = new VetoResponse("thought", null, true);
    ...
    void retriesRetryableFailureThenSucceeds() {
        ...
        VetoResponse expected = new VetoResponse("ok", null, true);
