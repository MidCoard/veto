 ...
 * the group's execution plan node by node - never as a submitted JSON blob - so the the host DAG
 * coordinator stays the DAG's source of truth: every call is one atomic, validated change
 ...
 *
 * <p>Both tools resolve the group from the the host call context (the caller leads exactly one
