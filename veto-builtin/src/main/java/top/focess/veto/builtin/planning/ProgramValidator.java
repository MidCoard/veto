...
public final class ProgramValidator {
    ...
    public static void validate(@NonNull ActionsProgram program) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (a instanceof GenerateAction g) {
                ...
                if (tier != null) {
                    try {
                        if (!PlanProgramSchema.MODEL_TIERS.contains(tier))
    ...
    public static void validateInputs(@NonNull ActionsProgram program) {
        ...
        while (!pending.isEmpty()) {
            ...
            var incomingBindings = incoming.get(index);
            if (incomingBindings == null)
                throw new IllegalStateException("Missing incoming bindings");
            Set<String> bound = new HashSet<>(incomingBindings);
