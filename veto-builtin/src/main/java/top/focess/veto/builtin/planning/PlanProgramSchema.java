...
public final class PlanProgramSchema implements ContextualInputSchemaSource {
    ...
    /** Builds the plan schema for the given catalog tools, without generic-tool support. */
    public static @NonNull JsonNode create(@NonNull List<ToolDefinition> tools) {
    ...
    public static @NonNull JsonNode create(
            @NonNull List<ToolDefinition> tools, @NonNull Set<String> javaRecordTools) {
    ...
    private static @NonNull JsonNode actionItemSchema(
        ...
        tools.stream()
                .sorted(Comparator.comparing(ToolDefinition::name, String.CASE_INSENSITIVE_ORDER))
                .forEach(
                        tool ->
                                variants.add(
                                        toolActionSchema(
    ...
    private static @NonNull ObjectNode toolActionSchema(
            @NonNull ToolDefinition tool, boolean javaNulls) {
