 ...
 * reports; Mates report via their Mate wrapper.
 *
 * <p>These are agent tools, so the Gateway returns {@code NotScreened}. Each tool resolves the
 * caller's group from the host call context (the calling agent leads exactly one group), so none of
 * them takes a {@code groupId} argument. Role gating is enforced by tool availability: {@code
 * create_group} is offered only to STANDALONE; {@code disband_group} / {@code inspect_group} only
 * to the Leader.
 */
public final class GroupTools {
    ...
    public static final class InspectGroup implements GroupControlTool<InspectGroup.Args> {
        ...
        public @NonNull String execute(
            ...
            return render(inspection.group(), inspection.messages(), since, capability);
        }
    }
    ...
    private static @NonNull String render(
            ...
            long since,
            @NonNull GroupControlCapability capability) {
