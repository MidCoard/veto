 ...
 * safe. The cadence defaults to 1s and is tunable via {@code group-tick-interval-ms in builtin
 * plugin configuration}. A group reaches {@code COMPLETED} when all nodes verify; the scheduler
 * then stops ticking it while the Leader answers the user. Adding a new node reactivates the same
 * group for subsequent ticks.
 */
public class GroupTickScheduler {

    private static final @NonNull Logger log =
            LoggerFactory.getLogger("top.focess.veto.builtin.group.GroupTickScheduler");
