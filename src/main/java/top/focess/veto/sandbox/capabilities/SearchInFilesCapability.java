...
/**
 * Atomic capability: search_in_files.
 * Searches for a pattern in text files within the sandbox.
...
public class SearchInFilesCapability implements AtomicCapability {
    ...
    public String execute(ToolExecutionRequest request) throws SecurityException {
        ...
        Path searchDir = dirPath != null && !dirPath.isBlank()
            ? resolveSafePath(dirPath) : sandboxRoot;
        ...
        try {
            PathMatcher matcher = fileGlob != null && !fileGlob.isBlank()
        ...
        String resultJson = results.stream()
            .map(r -> String.format(
        ...
        return String.format("{\"status\":\"ok\",\"pattern\":\"%s\",\"total\":%d,\"results\":%s}",
        ...
        try {
            ...
            for (int i = 0; i < lines.size() && results.size() < MAX_RESULTS; i++) {
                if (lines.get(i).toLowerCase().contains(pattern.toLowerCase())) {
                    ...
                    match.put("line", i + 1);
                    String truncated = lines.get(i).length() > 200
