...
/**
 * Atomic capability: diff_text_files.
 * Produces a unified diff between two text files within the sandbox.
 * Uses Java's built-in diff (no external diff tool required).
 */
@Component
public class DiffTextFilesCapability implements AtomicCapability {
    ...
    @Override
    public String getName() { return "diff_text_files"; }
    ...
    public String execute(ToolExecutionRequest request) throws SecurityException {
        ...
        try {
            ...
            String diff = computeUnifiedDiff(
                resolvedA.getFileName().toString(), resolvedB.getFileName().toString(),
                linesA, linesB);

            return String.format("{\"status\":\"ok\",\"fileA\":\"%s\",\"fileB\":\"%s\",\"diff\":\"%s\"}",
    ...
    private String computeUnifiedDiff(String nameA, String nameB, List<String> linesA, List<String> linesB) {
