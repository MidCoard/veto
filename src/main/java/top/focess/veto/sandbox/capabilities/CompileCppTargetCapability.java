...
public class CompileCppTargetCapability implements AtomicCapability {
    ...
    private static final List<String> ALLOWED_SOURCE_EXTENSIONS = List.of(".cpp", ".c", ".cc", ".cxx");
    ...
    public String execute(ToolExecutionRequest request) throws SecurityException {
        ...
        try {
            ...
            if (!finished) {
                ...
                return "{\"status\":\"error\",\"message\":\"Compilation timed out after " + COMPILE_TIMEOUT_SECONDS + "s\"}";
            }
    ...
    private void validateCompilerFlags(String flags) {
        ...
        for (String flag : flags.split("\\s+")) {
            if (flag.startsWith("-D") || flag.startsWith("-I") || flag.startsWith("-O")
                || flag.startsWith("-std=") || flag.startsWith("-f") || flag.equals("-g")) {
                continue;
            }
