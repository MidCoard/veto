...
public class SwarmOrchestrator {
    ...
    public SwarmOrchestrator(OrchestratorConfiguration config,
    ...
    public void init() {
        ...
        maintenanceScheduler.scheduleAtFixedRate(
            this::maintenanceCycle, 30, 30, TimeUnit.SECONDS);
    ...
    /**
     * Execute a tool execution request on an available worker.
     * Uses circuit breaker to prevent cascading failures.
     */
    public CompletableFuture<String> execute(ToolExecutionRequest task,
                                             Function<ToolExecutionRequest, String> executorFn) {
        ...
        log.info("C5 SwarmOrchestrator: Task #{} ('{}') assigned to Worker[{}]",
            taskNum, task.getCapabilityName(), worker.getId());
    ...
    public FileLockManager getFileLockManager() { return fileLockManager; }
}
