...
/**
 * REST controller for C6 Tool Execution via the Sandbox.
 * Submits atomic capability execution requests and retrieves results.
 */
@RestController
...
public class ToolController {
    ...
    private final ConcurrentHashMap<String, CompletableFuture<String>> pendingExecutions = new ConcurrentHashMap<>();
    ...
    public ResponseEntity<Map<String, Object>> executeTool(@RequestBody Map<String, Object> request) {
        ...
        Set<String> requiredCredentials = request.containsKey("requiredCredentials")
            ? Set.copyOf((java.util.List<String>) request.getOrDefault("requiredCredentials", java.util.List.of()))
            : Set.of();
        ...
        future.whenComplete((result, error) -> {
            ...
            if (error != null) {
                completedResults.put(id, Map.of(
                    "status", "error",
                    "id", id,
        ...
        return ResponseEntity.ok(Map.of(
            ...
            "id", id,
            "capabilityName", capabilityName,
            "requestStatus", execRequest.getStatus().name(),
            "availableCapabilities", toolSandbox.getRegisteredCapabilities(),
            "timestamp", Instant.now().toString()
        ));
    }
