...
/**
 * REST controller for C7 Veto Gateway operations.
 * Provides endpoints to process payloads through the veto gate and query gateway status.
 */
@RestController
...
public class VetoController {
    ...
    public ResponseEntity<Map<String, Object>> processPayload(@RequestBody Map<String, Object> request) {
        ...
        String dagPayloadId = (String) request.getOrDefault("dagPayloadId", UUID.randomUUID().toString());
        ...
        log.info("REST: /api/veto/process - payload={} bytes, source={}", payload.length(), componentSource);

        VetoGateway.VetoResult result = vetoGateway.processOutbound(payload, dagPayloadId, requestId, componentSource);
    ...
    public ResponseEntity<Map<String, Object>> checkPayload(@RequestBody Map<String, Object> request) {
        String payload = (String) request.getOrDefault("payload", "");
        if (payload.isBlank()) {
            return ResponseEntity.badRequest().body(Map.of(
                "status", "error",
        ...
        return ResponseEntity.ok(Map.of(
            "status", "ok",
            "safe", result.decision() == VetoGateway.VetoDecision.PASS,
            "decision", result.decision().name(),
            "redactionCount", result.redactionCount()
        ));
    }
