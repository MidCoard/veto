...
/**
 * REST controller for DAG task lifecycle management.
 * Provides endpoints to create, query, and manage DAG payload tasks.
 */
@RestController
...
public class TaskController {
    ...
    public ResponseEntity<Map<String, Object>> createTask(@RequestBody Map<String, Object> request) {
        ...
        DAGPayload payload = DAGPayload.builder()
            .id(request.containsKey("id") ? (String) request.get("id") : UUID.randomUUID().toString())
            .taskType(taskType)
    ...
    public ResponseEntity<Map<String, Object>> listTasks() {
        return ResponseEntity.ok(Map.of(
            ...
            "tasks", taskStore.values().stream().map(p -> Map.of(
                "id", p.getId(),
    ...
    public ResponseEntity<Map<String, Object>> cancelTask(@PathVariable("id") String id) {
        ...
        return ResponseEntity.ok(Map.of(
            "status", "ok",
            "id", id,
            "newStatus", DAGPayload.DAGPayloadStatus.CANCELLED.name(),
            "timestamp", Instant.now().toString()
        ));
    }
}
