 ...
 *  1. Extracts structural schemas
 *  2. Redacts sensitive literals (secrets, proprietary physics parameters)
 *  3. Enforces structural constraints before allowing data to flow to C3 (Communication Bus)
 *
 * Uses llama.cpp (quantized 1B-3B) with GBNF grammar-constrained decoding.
 */
@Service
public class VetoGateway {
    ...
    public void init() {
        if (!config.isEnabled()) {
            log.warn("C7 VetoGateway: DISABLED by configuration. ALL data will pass through unchecked!");
            return;
        }
        ...
        if (!slmStarted) {
            log.warn("C7 VetoGateway: SLM not available. Running in deterministic-only redaction mode.");
        }

        log.info("C7 VetoGateway: Initialized. interceptAllOutbound={}, redactSecrets={}, enforceConstraints={}",
            config.isInterceptAllOutbound(), config.isRedactSecrets(), config.isEnforceStructuralConstraints());
    }

    @PreDestroy
    public void shutdown() {
        llamaCppBridge.stop();
        log.info("C7 VetoGateway: Shut down. Processed {} vetoes, {} passes, {} redactions",
            totalVetoes.get(), totalPasses.get(), totalRedactions.get());
    }
     ...
     */
    public VetoResult processOutbound(String payload, String dagPayloadId,
                                      String requestId, String componentSource) {
        if (!config.isEnabled()) {
            return VetoResult.pass(payload, "Veto gateway disabled");
        }

        long startTime = System.currentTimeMillis();
        log.info("C7 VetoGateway: Processing outbound payload ({} bytes, source={})",
            payload.length(), componentSource);

        try {
            // Step 1: Deterministic redaction (first pass)
            SemanticRedactor.RedactionReport deterministicReport = semanticRedactor.deterministicRedact(payload);
            ...
            if (llamaCppBridge.isAvailable()) {
                ...
                llmGuidedPayload = semanticRedactor.semanticRedact(
            ...
            String reason;
            boolean wasRedacted = deterministicReport.wasModified()
                || !payload.equals(finalPayload);
            ...
            log.info("C7 VetoGateway: Decision={}, elapsed={}ms, redactions={}",
                decision, elapsed, deterministicReport.getTotalRedactions());
    ...
    /**
     * Enforce structural constraints on the payload.
     * Validates that the data adheres to project rules (e.g., normalized physics values).
     */
    private String enforceStructuralConstraints(String payload) {
        ...
        result = result.replaceAll("(?i)MIN_DB\\s*[:=]\\s*[-+]?\\d+(?:\\.\\d+)?",
            "[ENFORCED_NORMALIZED_PEAK]");

        // Enforce discrete physical solutions over continuous approximations
        result = result.replaceAll("(?i)continuous_approx\\s*[:=]\\s*\\w+",
    ...
    private String computeDiff(String original, String redacted) {
        ...
        if (origLines.length != redactedLines.length) {
            diff.append("(line count changed: ").append(origLines.length)
                .append(" -> ").append(redactedLines.length).append(")");
        }
    ...
    public long getTotalVetoes() { return totalVetoes.get(); }
    public long getTotalPasses() { return totalPasses.get(); }
    public long getTotalRedactions() { return totalRedactions.get(); }

    /**
     * The result of processing a payload through the Veto Gateway.
     */
    public record VetoResult(VetoDecision decision, String processedPayload,
                             String reason, int redactionCount) {
