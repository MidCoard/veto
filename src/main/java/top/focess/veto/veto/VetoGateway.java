...
public class VetoGateway {
  ...
  public VetoResult processOutbound(
    ...
    try {
      ...
      if (llamaCppBridge.isAvailable()) {
        ...
        llmGuidedPayload =
            semanticRedactor.semanticRedact(deterministicReport.redactedPayload(), slmAnalysis);
      } else {
      ...
      String reason;
      boolean wasRedacted = deterministicReport.wasModified() || !payload.equals(finalPayload);
  ...
  /**
   * Enforce structural constraints on the payload. Validates that the data adheres to project rules
   * (e.g., normalized physics values).
   */
  private String enforceStructuralConstraints(String payload) {
    ...
    result =
        result.replaceAll(
            "(?i)MIN_DB\\s*[:=]\\s*[-+]?\\d+(?:\\.\\d+)?", "[ENFORCED_NORMALIZED_PEAK]");

    // Enforce discrete physical solutions over continuous approximations
    result =
        result.replaceAll(
            "(?i)continuous_approx\\s*[:=]\\s*\\w+", "continuous_approx: [ENFORCED_DISCRETE]");
