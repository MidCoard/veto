 ...
 *
 * Works in concert with the local SLM (llama.cpp) for semantic understanding,
 * but also applies deterministic regex-based redaction as a first pass.
 */
@Component
public class SemanticRedactor {
    ...
    private static final List<RedactionRule> REDACTION_RULES = List.of(
        ...
        new RedactionRule(
            Pattern.compile("\\b(?:[a-zA-Z0-9-]+\\.)*(?:internal|local|private|corp|intranet)\\.[a-zA-Z]{2,}\\b",
        ...
        new RedactionRule(
            Pattern.compile("(?i)(?:~?/\\w+/)?(?:\\.ssh|credentials|secrets|passwd|keys)/?\\S*"),
    ...
    /**
     * Perform first-pass deterministic redaction on the payload.
     * Returns a RedactionReport with all findings and the redacted payload.
     */
    public RedactionReport deterministicRedact(String originalPayload) {
        ...
        for (RedactionRule rule : REDACTION_RULES) {
            ...
            if (count > 0) {
                ...
                log.debug("C7 Redactor: Redacted {} instances of type {}",
                    count, rule.type());
            }
        }
    ...
    /**
     * Perform semantic redaction using the local SLM for complex patterns.
     * This enriches the deterministic redaction with LLM-powered understanding.
     */
     ...
     */
    public record RedactionReport(String originalPayload, String redactedPayload, List<RedactionEntry> entries) {
        public int getTotalRedactions() { return entries.size(); }
        public boolean wasModified() { return !originalPayload.equals(redactedPayload); }
    }
