...
public class RoutingBusService {
    ...
    private final ConcurrentMap<String, CompletableFuture<DAGPayload>> pendingFutures = new ConcurrentHashMap<>();

    public RoutingBusService(WebSocketBus webSocketBus, BusConfiguration config, ObjectMapper objectMapper) {
    ...
    public void init() {
        ...
        webSocketBus.registerMessageRoute("fallback", payload -> {
            log.debug("C3 Bus: Fallback message received ({} bytes)", payload.length());
        });

        log.info("C3 RoutingBusService: Initialized. WS port={}, gRPC port={}",
            config.getWebsocket().getPort(), config.getGrpc().getPort());
    }
