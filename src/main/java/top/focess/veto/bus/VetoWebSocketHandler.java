...
public class VetoWebSocketHandler extends TextWebSocketHandler {
  ...
  protected void handleTextMessage(WebSocketSession session, TextMessage message) {
    ...
    log.debug(
        "WS Bus: Received msg #{} from '{}' ({} bytes)", seq, session.getId(), payload.length());
    ...
    try {
      ...
      sendJson(
          session,
          Map.of(
              "type", "error", "message", "Invalid message format: " + e.getMessage(), "seq", seq));
    }
  }
  ...
  private void handleSubscribe(WebSocketSession session, Map<String, Object> msg) {
    ...
    sendJson(
        session,
        Map.of("type", "subscribed", "topic", topic, "timestamp", Instant.now().toString()));
  }
  ...
  private void handleUnknownType(WebSocketSession session, String payload, long seq) {
    log.debug(
        "WS Bus: Unknown message type from '{}' ({} bytes)", session.getId(), payload.length());
    sendJson(
        session,
        Map.of("type", "echo", "data", payload, "seq", seq, "timestamp", Instant.now().toString()));
  }
  ...
  public void streamDAGResult(DAGPayload payload) {
    try {
      String json =
          objectMapper.writeValueAsString(
              Map.of(
                  "type", "dag.result", "payload", payload, "timestamp", Instant.now().toString()));
      broadcastRaw(json);
    } catch (Exception e) {
