...
/**
 * Server-side WebSocket handler for the Veto Bus (/ws/veto/bus).
 * Clients (UI, MCP servers, workers) connect here for real-time payload streaming.
 * Routes DAG payloads, streams veto results, and handles heartbeats.
 */
@Component
public class VetoWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) {
        ...
        log.debug("WS Bus: Received msg #{} from '{}' ({} bytes)", seq, session.getId(), payload.length());
        ...
        try {
            ...
            log.warn("WS Bus: Failed to parse message from '{}': {}", session.getId(), e.getMessage());
    ...
    private void handleVetoProcess(WebSocketSession session, Map<String, Object> msg, long seq) {
        ...
        log.info("WS Bus: Veto processing from '{}' - payload={} bytes", session.getId(), rawPayload.length());

        VetoGateway.VetoResult result = vetoGateway.processOutbound(rawPayload, dagPayloadId, requestId, componentSource);
    ...
    private void handleSubscribe(WebSocketSession session, Map<String, Object> msg) {
        ...
        sendJson(session, Map.of("type", "subscribed", "topic", topic, "timestamp", Instant.now().toString()));
    }
    ...
    private void handleUnknownType(WebSocketSession session, String payload, long seq) {
        log.debug("WS Bus: Unknown message type from '{}' ({} bytes)", session.getId(), payload.length());
        sendJson(session, Map.of("type", "echo", "data", payload, "seq", seq, "timestamp", Instant.now().toString()));
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        ...
        sessionRoutes.remove(session.getId());
        log.info("WS Bus: Client '{}' disconnected (code={}, reason='{}')",
