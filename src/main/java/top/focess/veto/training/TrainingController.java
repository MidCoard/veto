 ...
 *   POST /api/v1/training/cancel    -  Cancel current training
 *   GET  /api/v1/training/progress  -  Get training progress
 *   POST /api/v1/training/deploy    -  Deploy a trained model to the gateway
 *   GET  /api/v1/training/status    -  Get overall training system status
 */
@RestController
...
public class TrainingController {
     ...
     * Generates data -> fine-tunes -> converts to GGUF -> evaluates -> auto-deploys.
     */
    @PostMapping("/start")
    public ResponseEntity<Map<String, Object>> startTraining() {
        ...
        if (started) {
            ...
            return ResponseEntity.status(500).body(Map.of(
                "success", false,
                "message", "Failed to start training pipeline. Check configuration.",
                "error", trainingManager.getProgress().getErrorMessage()
            ));
        }
    }
    ...
    /**
     * Deploy a trained model to the path expected by LlamaCppBridge.
     * Optionally restarts the bridge with the new model.
     *
     * Request body:
     *   { "modelPath": "./training/models/veto-slm-q4_k_m.gguf" }
     */
    @PostMapping("/deploy")
    public ResponseEntity<Map<String, Object>> deployModel(@RequestBody Map<String, String> request) {
        ...
        if (deployed) {
            ...
            return ResponseEntity.ok(Map.of(
                ...
                "modelPath", modelPath,
                "targetPath", config.getModelOutputDir() + "/" + config.getDefaultGgufName()
            ));
        } else {
            return ResponseEntity.status(500).body(Map.of(
                "success", false,
                "message", "Failed to deploy model. Check that the file exists.",
                "modelPath", modelPath
            ));
        }
    }
