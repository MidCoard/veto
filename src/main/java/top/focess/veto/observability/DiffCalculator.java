...
public class DiffCalculator {
    ...
    public String generateSummaryReport(DiffResult diff) {
        ...
        for (DiffLine line : diff.lineDiffs) {
            ...
            sb.append(String.format("L%d: -%s%n    +%s%n", line.lineNumber, truncate(line.original), truncate(line.redacted)));
            shown++;
        }
     ...
     */
    public record DiffResult(int totalChanges, int charsChanged, int lineCountChange,
                             String summary, List<DiffLine> lineDiffs) {
    ...
    public record DiffLine(int lineNumber, String original, String redacted) {
        public boolean isModified() { return !original.equals(redacted); }
        public boolean isAdded() { return original.isEmpty(); }
        public boolean isRemoved() { return redacted.isEmpty(); }
    }
}
