...
public class TamperProofStore {
    ...
    public void initialize() {
        try {
            ...
            loadChainTail();
            log.info("C9 TamperProofStore: Initialized at '{}'. Chain tail hash: {}",
                auditDir, chainTailHash.isEmpty() ? "(new chain)" : chainTailHash.substring(0, 16) + "...");
        } catch (IOException e) {
    ...
    /**
     * Append an audit record to the tamper-proof store.
     * The record is encrypted and written to the daily audit file.
     * The hash chain is maintained in memory and persisted in a compact index.
     */
    public synchronized void append(AuditRecord record) {
        try {
            ...
            if (!chainTailHash.isEmpty() && !record.getPreviousRecordHash().equals(chainTailHash)) {
                log.warn("C9 TamperProofStore: Hash chain broken! Expected tail '{}', got '{}'",
                    chainTailHash, record.getPreviousRecordHash());
            }
            ...
            log.debug("C9 TamperProofStore: Appended record '{}' (action={})", record.getId(), record.getAction());
    ...
    public ChainVerificationResult verifyChain() {
        ...
        try {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length >= 3) {
                        ...
                        if (!prevHash.equals(previousHash)) {
                            violations.add("Chain break at record " + recordId
                                + ": expected prev=" + previousHash.substring(0, Math.min(8, previousHash.length()))
                                + ", got " + prevHash.substring(0, Math.min(8, prevHash.length())));
                        }
            ...
            return new ChainVerificationResult(recordsChecked, false,
                "I/O error during verification: " + e.getMessage());
        }

        boolean intact = violations.isEmpty();
        String summary = intact
            ? String.format("Chain intact: %d records verified", recordsChecked)
            : String.format("Chain BROKEN: %d violations in %d records", violations.size(), recordsChecked);
     ...
     * Requires the store encryption key.
     */
    public List<String> exportRecords(LocalDate from, LocalDate to) {
        List<String> records = new ArrayList<>();
        try {
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(auditDir, STORE_FILE_PREFIX + "*" + STORE_FILE_SUFFIX)) {
    ...
    private void appendToIndex(AuditRecord record) throws IOException {
        Path indexFile = auditDir.resolve(INDEX_FILE);
        String indexLine = String.join("|",
            record.getId(), record.getCurrentHash(), record.getPreviousRecordHash(),
            record.getTimestamp().toString(), record.getAction().name());
        Files.writeString(indexFile, indexLine + "\n",
    ...
    private LocalDate extractDate(String fileName) {
        try {
            String dateStr = fileName
                .replace(STORE_FILE_PREFIX, "")
                .replace(STORE_FILE_SUFFIX, "");
    ...
    public String getChainTailHash() { return chainTailHash; }

    /**
     * Result of an audit chain verification.
     */
    public record ChainVerificationResult(int recordsChecked, boolean chainIntact, String summary) {}
}
