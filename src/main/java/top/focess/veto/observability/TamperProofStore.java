  ...
  public void initialize() {
    try {
      ...
      log.info(
          ...
          auditDir,
          chainTailHash.isEmpty() ? "(new chain)" : chainTailHash.substring(0, 16) + "...");
    } catch (IOException e) {
  ...
  public ChainVerificationResult verifyChain() {
            ...
            if (!prevHash.equals(previousHash)) {
              violations.add(
                  "Chain break at record "
                      + recordId
                      + ": expected prev="
                      + previousHash.substring(0, Math.min(8, previousHash.length()))
                      + ", got "
    ...
    String summary =
        intact
            ...
            : String.format(
                "Chain BROKEN: %d violations in %d records", violations.size(), recordsChecked);
  ...
  public List<String> exportRecords(LocalDate from, LocalDate to) {
    ...
    try {
      try (DirectoryStream<Path> stream =
          Files.newDirectoryStream(auditDir, STORE_FILE_PREFIX + "*" + STORE_FILE_SUFFIX)) {
