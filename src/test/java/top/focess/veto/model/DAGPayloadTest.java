...
class DAGPayloadTest {
  ...
  void testWithUpdatedParameters() {
    DAGPayload payload = DAGPayload.builder().taskType("test").parameter("key1", "value1").build();
  ...
  void testImmutableParameters() {
    DAGPayload payload = DAGPayload.builder().taskType("test").parameter("key", "value").build();
    assertThrows(
        UnsupportedOperationException.class, () -> payload.getParameters().put("new", "value"));
  }
