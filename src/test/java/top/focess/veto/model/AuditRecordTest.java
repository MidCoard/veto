...
class AuditRecordTest {
    ...
    void testCreateAndVerifyIntegrity() {
        AuditRecord record = new AuditRecord(
            "dag-1", "req-1", "C7-Veto",
            "raw data with secrets", "redacted data (safe)",
    ...
    void testBrokenChainDetection() {
        AuditRecord first = new AuditRecord("d1", "r1", "C9", "a", "b", "diff", "",
            AuditRecord.AuditAction.SYSTEM_ERROR, false);
        AuditRecord broken = new AuditRecord("d1", "r1", "C9", "a", "b", "diff", "wrong_hash",
            AuditRecord.AuditAction.SYSTEM_ERROR, false);
    ...
    void testDifferentActions() {
        for (AuditRecord.AuditAction action : AuditRecord.AuditAction.values()) {
            AuditRecord record = new AuditRecord("d1", "r1", "C9", "a", "b", "diff", "",
                action, false);
