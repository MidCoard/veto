...
class VetoApplicationTests {
  ...
  void vetoGatewayProcessesCleanPayload() {
    ...
    assertEquals(
        cleanPayload, result.processedPayload(), "Clean payload should pass through unchanged");
  }
  ...
  void vetoGatewayRedactsSensitiveData() {
    ...
    VetoGateway.VetoResult result =
        vetoGateway.processOutbound(sensitivePayload, "it-dag-2", "it-req-2", "IT-Integration");
  ...
  void vetoGatewayDisabledState() {
    ...
    assertEquals(
        VetoGateway.VetoDecision.PASS, result.decision(), "Disabled gateway should always pass");
    assertEquals(
  ...
  void grammarEngineLoaded() {
    ...
    assertTrue(
        defaultGrammar.contains("veto_decision"), "Default grammar should define veto_decision");
  ...
  void restEndpointVetoProcessRejectsEmptyPayload() {
    ...
    assertEquals(
        HttpStatus.BAD_REQUEST, response.getStatusCode(), "Empty payload should return 400");
  }
  ...
  void restEndpointTaskLifecycle() {
    ...
    HttpEntity<Map<String, Object>> createRequest =
        new HttpEntity<>(
            Map.of("taskType", "integration_test", "parameters", Map.of("key", "value")), headers);
    ...
    ResponseEntity<Map> notFoundResponse =
        restTemplate.getForEntity("http://localhost:" + port + "/api/tasks/nonexistent", Map.class);
