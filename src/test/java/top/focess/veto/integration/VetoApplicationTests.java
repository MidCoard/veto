...
/**
 * Full Spring Boot integration tests for Project Veto.
 * Verifies context loading, DI wiring, and the core Veto pipeline.
 */
@SpringBootTest(
...
class VetoApplicationTests {
    ...
    void vetoGatewayProcessesCleanPayload() {
        ...
        VetoGateway.VetoResult result = vetoGateway.processOutbound(
            cleanPayload, "it-dag-1", "it-req-1", "IT-Integration");

        assertNotNull(result, "Result should not be null");
        assertTrue(result.isAllowed(), "Clean payload should be allowed");
        assertEquals(cleanPayload, result.processedPayload(), "Clean payload should pass through unchanged");
    }

    @Test
    void vetoGatewayRedactsSensitiveData() {
        String sensitivePayload = "Server: 192.168.1.100, API key: abcdefghijklmnopqrstuvwxyz0123456789ABCDEF";
        VetoGateway.VetoResult result = vetoGateway.processOutbound(
        ...
        assertFalse(result.processedPayload().contains("192.168.1.100"),
            "Redacted payload should not contain original IP");
        assertFalse(result.processedPayload().contains("abcdefghijklmnopqrstuvwxyz0123456789ABCDEF"),
    ...
    void vetoGatewayDisabledState() {
        ...
        VetoGateway disabledGateway = new VetoGateway(
            disabledConfig,
        ...
        VetoGateway.VetoResult result = disabledGateway.processOutbound(
            sensitive, "it-dag-3", "it-req-3", "IT-Disabled");

        assertEquals(VetoGateway.VetoDecision.PASS, result.decision(),
            "Disabled gateway should always pass");
        assertEquals(sensitive, result.processedPayload(),
            "Disabled gateway should not modify payload");
    }
    ...
    void semanticRedactorPatternDetection() {
        String multiSecret = "IP: 10.0.0.50, Email: admin@internal.corp, SSH: -----BEGIN OPENSSH PRIVATE KEY-----test-----END OPENSSH PRIVATE KEY-----";
    ...
    void grammarEngineLoaded() {
        ...
        assertTrue(defaultGrammar.contains("veto_decision"), "Default grammar should define veto_decision");
    ...
    void restEndpointVetoProcess() {
        ...
        HttpEntity<Map<String, Object>> request = new HttpEntity<>(Map.of(
            "payload", "Test IP: 10.0.0.55 for processing"
        ), headers);
    ...
    void restEndpointToolCapabilities() {
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode(), "Capabilities endpoint should return 200");
        assertNotNull(response.getBody());
        assertTrue(response.getBody().containsKey("capabilities"),
    ...
    void restEndpointTaskLifecycle() {
        ...
        ResponseEntity<Map> createResponse = restTemplate.postForEntity(createUrl, createRequest, Map.class);
        ...
        ResponseEntity<Map> deleteResponse = restTemplate.exchange(
            getUrl, HttpMethod.DELETE, null, Map.class);
